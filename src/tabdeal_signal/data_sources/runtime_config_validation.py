    ...
    if requires_explicit_host:
        ...
        try:
            config = explicit_runtime_endpoint_config(
                source_id,
                "https://approved.example/validation",
                ("approved.example",),
            )
            if config.source_id != source_id:
                errors.append("explicit runtime config returned wrong source ID")
        except (ValueError, KeyError) as exc:
            errors.append(f"explicit runtime host configuration failed: {exc}")
    else:
        for host in known_hosts:
            try:
