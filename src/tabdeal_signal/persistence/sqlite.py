...
class SQLiteDecisionPersistence:
    ...
    def _validate_schema_state(self) -> None:
        ...
        invalid_status = self._connection.execute(
            ...
            WHERE status NOT IN ('PENDING', 'PROCESSING', 'RETRY', 'SENT', 'DEAD_LETTER')
            LIMIT 1
            """
        ).fetchone()
        ...
        invalid_attempt = self._connection.execute(
            ...
            WHERE typeof(attempt_count) != 'integer' OR attempt_count < 0
            LIMIT 1
            """
        ).fetchone()
        ...
        inconsistent_lease = self._connection.execute(
            ...
            WHERE (owner_id IS NULL) != (lease_token IS NULL)
            LIMIT 1
            """
        ).fetchone()
