...
def _request(tmp_path, status=DecisionStatus.BLOCKED, key="k-1"):
    ...
    decision = FinalDecision(status=status, reason_code="TEST_BLOCK" if status == DecisionStatus.BLOCKED else "")
...
def test_collision_is_rejected(tmp_path):
    ...
    try:
        ...
        with pytest.raises(PersistenceCollisionError):
            db.persist(_request(tmp_path, key="same-key").__class__(
                context=_request(tmp_path, key="same-key").context,
                decision=FinalDecision(status=DecisionStatus.BLOCKED, reason_code="DIFFERENT"),
