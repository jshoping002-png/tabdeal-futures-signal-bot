...
def test_strategy_evaluation_rejects_snapshot_data_unavailable_at_reference_time():
    ...
    try:
        MarketSnapshot(
            ...
            reference_time,
            tuple(sorted(request_value.snapshot.candles + (future_candle,), key=lambda c: (c.symbol, c.timeframe, c.open_time))),
        )
...
def test_strategy_evaluation_blocks_when_15m_breakout_is_missing():
    ...
    modified_candles = tuple(
        Candle(
            ...
            c.high,
            c.low,
            19.0 if c.timeframe == "15m" and c.open_time == BASE + timedelta(hours=57, minutes=15) else c.close,
            c.volume,
        )
        for c in snapshot.candles
    )
