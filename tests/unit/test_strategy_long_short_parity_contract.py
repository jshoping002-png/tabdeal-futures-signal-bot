...
def short_request():
    ...
    for i, high, low in ((2, 20.0, 10.0), (4, 25.0, 12.0), (6, 15.0, 11.0), (8, 20.0, 8.0)):
        candles_4h[i] = make_candle("4h", candles_4h[i].open_time, timedelta(hours=4), high=high, low=low)

    candles_1h = [make_candle("1h", BASE + timedelta(hours=48) + i * timedelta(hours=1)) for i in range(7)]
    ...
    candles_15m = [make_candle("15m", BASE + timedelta(hours=56) + i * timedelta(minutes=15)) for i in range(7)]
