...
def build_request():
    ...
    all_candles = tuple(sorted(candles_4h + candles_1h + candles_15m, key=lambda c: (c.symbol, c.timeframe, c.open_time)))
    reference_time = max(c.close_time for c in all_candles) + timedelta(minutes=1)
    snapshot = MarketSnapshot("snapshot-trend-neutral", "test-source", reference_time, all_candles)
...
def test_strategy_evaluation_blocks_when_4h_trend_is_neutral():
    ...
    # Collapse the second confirmed 4H swing high so the higher-timeframe
    # structure no longer qualifies as a LONG trend.
